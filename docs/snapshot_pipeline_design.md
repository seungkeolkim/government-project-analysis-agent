...
- **첨부 파일 GC 스크립트** (`scripts/python/gc_orphan_attachments.py`) — 본 테이블에
...
scripts/python/gc_orphan_attachments.py              # §11 GC 스크립트 (CLI entry point)
...
| 00041-5 | scripts/python/gc_orphan_attachments.py | §11 |
...
### §11.2 스크립트 시그니처 (`scripts/python/gc_orphan_attachments.py`)
...
위 11 항은 task 00041 종료 시 모두 회귀 통과 확인됨 (해당 회귀의 1회성 verification 노트는 보존하지 않는다).

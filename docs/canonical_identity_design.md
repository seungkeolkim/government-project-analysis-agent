...
**`title` 정규화 규칙** (`app/canonical.py: _normalize_official_title`)

> **task 00151 변경 (2026-05-28)**: 이전 버전의 NTIS suffix 절단 단계를 제거했다.
> 동일 ancmNo 아래 NTIS 가 별개 sub-business 공고를 별도 row 로 게시하는 경우
> (예: ann 173/174, ancmNo 2026-0627호) suffix 가 sub-business 식별자 역할을 하므로
> 무조건 절단하면 서로 다른 공고가 같은 canonical_key 로 잘못 합쳐진다. 새 정규화는
> full title 을 보존하고, NTIS suffix 가 부착된 IRIS↔NTIS 1:1 cross-source 묶음은
> `_apply_canonical` 의 fallback 매칭이 별도로 처리한다 (아래 §4-1b 신설).

1. NFKC 유니코드 정규화
2. 모든 공백 문자 제거 (leading/trailing whitespace + 단어 사이 공백 변이 흡수)
3. 길이 제한 미적용 — 정확일치 비교가 전제이므로 truncation 불필요

### 4-1b. cross-source fallback 매칭 (task 00151 신설)

canonical_key 정확일치로 묶이지 않는 IRIS↔NTIS cross-source 쌍을 묶기 위해,
`app/db/repository.py::_find_cross_source_canonical_group` 가 official scheme 의
보조 매칭 단계로 동작한다. 매칭 절차:

1. 현재 announcement 의 canonical_key 에서 ancmNo prefix(`official:<X>::`) 를 추출.
2. 같은 prefix 를 가진 is_current=True 의 다른 announcement 들을 조회.
3. **title strip 기준 분류** — `strip_ntis_business_suffix` (NFKC → `\s*_\([0-9]{4}\).*$`
   절단 → 공백 제거) 가 본 row 와 동치인 후보만 매칭 후보로 본다. source_type 별로
   `same_source_title_matches` / `cross_source_title_matches` 두 리스트로 나눈다.
4. **same-source 동치 가드** — `same_source_title_matches` 가 비어 있지 않으면
   매칭 거부. 동일 source 의 다른 row 가 *같은 strip-title* 을 가졌다는 뜻은 단일
   umbrella 가 동일 source 의 다중 sub-business 와 모두 매칭되는 다대일 케이스라
   어느 한쪽으로 합치는 결정이 모호하다. (173/174 자체는 `cross_source_title_matches`
   가 0건이라 후술 가드에서 자연 거부됨 — 본 가드는 그 변형 시나리오 방어용.)
5. **cross-source 단일 동치 가드** — `cross_source_title_matches` 가 정확히 1건이고
   그 partner 의 canonical_group_id 가 채워져 있으면 그 group 을 공유. 그 외는 거부.

매칭 성공 시 partner 의 canonical_group_id 만 공유한다. announcement 의 canonical_key
본문(suffix 포함) 은 partner 와 달라도 무방하다. 단위/통합 회귀 테스트는
`tests/test_canonical.py`, `tests/test_canonical_cross_source_fallback.py` 에 고정.

**title strip 기준 분류로 정제한 이유 (task 00151-2)**

00151-1 의 초기 구현은 같은 ancmNo 의 *모든 same-source* 후보를 거부 조건으로 봤다.
그러나 운영 DB 의 group 121 (ancmNo 0601호) 에는 IRIS umbrella(ann 133) + NTIS
"-플랫폼형"(ann 156, 비-NTIS-pattern suffix) + NTIS "_(2026)소재HUB"(ann 162, NTIS
pattern suffix) 가 있어, ann 162 의 cross-source 매칭이 ann 156 의 존재로 거부되며
group 121 의 IRIS+NTIS 쌍이 깨졌다. title strip 기준으로 정제하면 ann 156 의 strip
(`...3차재공모-플랫폼형`) 이 ann 162 의 strip(`...3차재공모`) 과 달라 가드에서 제외되고,
ann 162 ↔ ann 133 의 1:1 매칭만 유지된다. 173/174 패턴은 IRIS 가 없어
`cross_source_title_matches` 가 0건이라 분리가 그대로 유지된다.
...
| 원문 title (task 00151 이후 — full title 보존) | 정규화 title |
|------------------------------------------------|--------------|
| `'2026년도 대형가속기 … 신규과제 재공모 _(2026)2026년도 대형가속기정책센터 신규과제 재공모'` | `'2026년도대형가속기…신규과제재공모_(2026)2026년도대형가속기정책센터신규과제재공모'` (suffix 보존 + 공백 제거 — cross-source 매칭은 §4-1b 의 fallback 이 담당) |
| `'2026년도 대형가속기 … 신규과제 재공모 '` (IRIS, trailing space) | `'2026년도대형가속기…신규과제재공모'` (공백 제거) |
...
### 11-7. 00151 — NTIS sub-business suffix 보존 (2026-05-28)

운영 DB 에서 announcements.id=173, 174 가 동일 canonical_group_id=152 로 잘못 묶인
사례가 발견됐다. 두 row 는 같은 ancmNo `과학기술정보통신부 공고 제2026-0627호` 아래
NTIS 가 게시한 서로 다른 sub-business 공고 (`국가전략기술미래소재기술개발(미래소재)` vs
`글로벌공급망첨단소재기술개발-나노커넥트`) 였으나, 당시 `_normalize_official_title` 의
`\s*_\([0-9]{4}\).*$` 절단 단계가 두 title 의 sub-business 식별자(suffix) 를 모두
삭제해 정규화 결과가 동일해진 것이 원인.

**수정 (canonical 4-1 + 4-1b 참조)**: `_normalize_official_title` 의 suffix 절단을
제거하여 canonical_key 가 full title 을 보존한다. 기존 IRIS↔NTIS 정상 cross-source
묶음(예: 그룹 17 ann 17+35 — IRIS title + NTIS title with `_(2026)대형가속기정책센터…`
suffix; 그룹 121 ann 133+162) 은 §4-1b 의 fallback 매칭이 같은 canonical_group_id 로
유지한다. fallback 은 후보를 title strip 기준으로 분류한 뒤 (a) same-source 동치
후보가 존재하면 매칭 거부, (b) cross-source 동치 후보가 정확히 1건이고 그 partner 의
canonical_group_id 가 채워져 있을 때만 매칭. 173/174 같은 same-source 다중
sub-business 케이스는 cross-source 동치 후보가 0건이라 자연 거부된다.

**앞으로의 동작**:
- 동일 ancmNo + 동일 source 의 sub-business 다중 row (IRIS 없음, 예: 173/174) →
  각자 별개 group (분리).
- 동일 ancmNo + IRIS 1건 + NTIS 1건 (NTIS 가 `_(YYYY)X` suffix 부착 포함) →
  fallback 매칭으로 같은 group (유지).
- 동일 ancmNo + IRIS 1건 + NTIS 다수 (한 NTIS 만 IRIS strip 과 동치 — 예: 그룹 121
  변형 ann 133/156/162) → strip 동치인 IRIS+NTIS 쌍만 같은 group, 비동치 NTIS 는
  별 group.
- 동일 ancmNo + IRIS 1건 + NTIS 다수 (모두 IRIS strip 과 동치) → 셋 다 별 group
  (보수적, multi-match 모호).

**00151-2 운영 DB 정정**: `scripts/python/recompute_canonical_00151.py` 가 새 로직으로
기존 DB 를 일괄 재계산. 173/174 분리 적용, 기존 cross-source 쌍 유지 (group 17, 121
포함), singleton 12 건의 stale canonical_key 정정 (list_row.ancm_no 갱신 후 키 갱신
누락 등), orphan canonical_projects 20 건 삭제. 멱등 확인.

회귀 테스트: `tests/test_canonical.py` 의 173/174 분리 케이스(DIFFERENT_KEY_CASES) 및
`tests/test_canonical_cross_source_fallback.py` 의 그룹 17/121 fallback 매칭, IRIS
umbrella + NTIS 다중 sub-business, NTIS 선등록 순서 변이 등 6개 시나리오 케이스.

...
3. **same source_type 가드** — 후보 중 같은 source_type 이 1건이라도 있으면 매칭
   거부. 동일 ancmNo 아래 동일 source 의 sub-business 분기 가능성(173/174 패턴)을
   차단한다.
4. **cross-source 단일 후보 가드** — 다른 source_type 후보가 정확히 1건일 때만 매칭.
5. **NTIS suffix 절단 후 title 동치성** — `strip_ntis_business_suffix` (NFKC →
   `\s*_\([0-9]{4}\).*$` 절단 → 공백 제거) 결과가 양측에서 같아야 매칭.

위 조건을 모두 만족하면 partner 의 canonical_group_id 를 그대로 공유한다. 매칭된
announcement 의 canonical_key 본문(suffix 포함) 자체는 partner 와 다를 수 있으나,
canonical_group_id 만 공유한다. 단위/통합 회귀 테스트는
...
유지한다. fallback 은 (a) same source_type 의 다른 row 가 같은 ancmNo prefix 에
존재하면 매칭 거부, (b) cross-source 후보가 정확히 1건일 때만 매칭, (c) NTIS suffix
절단 후 title 동치성 비교를 모두 만족할 때만 동작 — 173/174 같은 same-source
다중 sub-business 케이스는 (a) 가드로 차단된다.

**앞으로의 동작**:
- 동일 ancmNo + 동일 source 의 sub-business 다중 row → 각자 별개 group (분리).
- 동일 ancmNo + IRIS 1건 + NTIS 1건 (NTIS 가 `_(YYYY)X` suffix 부착 포함) →
  fallback 매칭으로 같은 group (유지).
- 동일 ancmNo + IRIS 1건 + NTIS 2건 이상 → IRIS / 각 NTIS sub-business 모두 별개
  group (안전 측). 이는 §11-1 의 N:1 의도와 약간 다른 보수적 선택.

회귀 테스트: `tests/test_canonical.py` 의 173/174 분리 케이스(DIFFERENT_KEY_CASES) 및
`tests/test_canonical_cross_source_fallback.py` 의 그룹 17/121 fallback 매칭 케이스.

...
> - [docs/canonical_identity_design.md](canonical_identity_design.md) — canonical_key, key_scheme 상세, 알려진 false-positive(§11)
...
제목·기관·연도 유사도로 잘못 묶인 false-positive 가 존재할 수 있다.

알려진 false-positive 패턴과 해소 경로는
[docs/canonical_identity_design.md §11](canonical_identity_design.md) 참조.
잔존 false-positive 의 사용자 측 해소(merge/split)는 Phase 5 `canonical_overrides` 범위이며,
현재 Phase 3b 에서는 **읽기 전용 표시**만 한다.
이 한계는 동일과제 expand UI 의 `[유사]` 배지로 사용자에게 명시한다.
...
00036-1 (본 문서 — 설계 확정)
    ↓
00036-2  canonical 묶음 품질 감사 (readonly) — 결과는 docs/canonical_identity_design.md §11 으로 흡수됨
    ↓

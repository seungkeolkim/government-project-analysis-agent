...
우선 순위:
1. official 키  — 정부가 공식 발급한 공고번호(ancmNo 등)를 NFKC 정규화 후 공백 제거.
...
def compute_canonical_key(
    *,
    ...
    for candidate in official_key_candidates:
        normalized = _normalize_official_key(candidate)
        if normalized:
            return CanonicalKeyResult(
                canonical_key=f"official:{normalized}",

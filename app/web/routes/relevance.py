...
task 00085 — 조직 단위 판정 확장:
    - POST/DELETE body 에 ``organization_id`` (필수) 를 지정한다.
...
task 00093 — 개인 판정 슬롯 제거:
    - organization_id 는 항상 필수(정수). 개인 판정(IS NULL) 경로는 완전히 제거됨.
    - GET /mine 는 mine_organization 만 반환한다.
...
class RelevanceJudgmentIn(BaseModel):
    ...
    Attributes:
        ...
        organization_id: 조직 PK (필수). 본인 소속이 아닌 조직 PK 가 들어오면
            422 로 응답한다 (사용자 원문 검증 항목 3).
    """
...
class RelevanceJudgmentDeleteIn(BaseModel):
    """DELETE /canonical/{id}/relevance 요청 본문 스키마.

    조직 row 를 삭제하려면 본문에 ``{"organization_id": <int>}`` 를 보낸다.

    Attributes:
        organization_id: 삭제 대상 조직 PK (필수).
    """
...
def set_relevance(
    ...
    with session_scope() as session:
        ...
        # 본인 소속 조직 검증 — 사용자 원문 검증 #3 / #4.
        # 무소속 사용자의 경우 get_user_organization_ids 가 빈 list 를 반환하므로
        # 같은 422 분기로 자연스럽게 차단된다.
        user_organization_ids = get_user_organization_ids(session, current_user.id)
...
def remove_relevance(
    canonical_id: int,
    body: RelevanceJudgmentDeleteIn,
    current_user: User = Depends(current_user_required),
) -> JSONResponse:
    ...
    삭제 대상 트리플:
        ...
        자연스럽게 잡히지 않고 404 로 응답된다 (사용자 원문 검증 #6).
...
def get_history(
    ...
    """판정 히스토리를 최신순으로 반환한다 (조직 row 모두 포함).
...
def get_my_relevance_judgments(
    ...
    """본인이 해당 canonical 에 작성한 조직 판정 목록을 반환한다.

    task 00089 — 관련성 모달 하단 '내 판정 목록' 섹션의 초기 로딩 및 X 삭제 후
    목록 재갱신에 사용한다. others 는 포함하지 않는다.

    task 00093 — 개인 판정 슬롯 제거. mine_organization 만 반환한다.

    응답 items 의 정렬은 decided_at DESC 순서로 나열한다.

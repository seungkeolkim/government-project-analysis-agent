...
//     드롭다운/verdict/reason 3 필드를 prefill 한다.
//   - POST/DELETE 요청 body 에 organization_id 가 함께 전송된다 (필수).
//   - 상세 페이지 .rj-detail-edit-btn / .rj-detail-create-btn / .rj-detail-delete-btn
//     클릭 시 모달을 같은 흐름으로 띄운다 + 단독 삭제는 별도 confirm 후 DELETE.
//
// task 00093 변경 사항:
//   - 판정 주체 라디오(개인/조직) 분기 완전 제거 — 조직 판정만 존재.
//   - organization_id 는 항상 필수. 무소속 사용자는 저장 버튼 자체가 disabled.
//
...
(function () {
    ...
    // 조직 선택 행 + 드롭다운 (무소속 사용자는 DOM 에 없을 수 있음).
     ...
     *
     * - 단일 조직 모드 → data-single-organization-id (정수)
     * - 복수 조직 모드 → select 의 현재 값 (정수)
     * - organizationRow 없음(무소속) → null
     */
     ...
     *     data-my-origin (organization | none), data-my-organization-id 를
     *     통해 폼이 prefill 된다.
     *     - 목록의 .rj-wrap (배지 컨테이너) — 본인 큰 배지 row 의 verdict 로 prefill.
     *     - 상세 페이지의 .rj-detail-edit-btn — 그 행의 특정 조직 row 로 prefill.
     *     - 상세 페이지의 .rj-detail-create-btn — 빈 폼 (verdict 미선택).
     */
    window.openRelevanceModal = function (invokerEl) {
        ...
        // 복수 조직 드롭다운 prefill — organization_id 가 들어왔을 때.
        if (organizationSelect && organizationIdRaw) {
    ...
    form.addEventListener('submit', function (e) {
        ...
        .then(function (resp) {
            ...
            if (currentInvoker.classList && currentInvoker.classList.contains('rj-wrap')) {
                updateBadgeWrap(currentInvoker, selectedVerdict, reason, organizationIdValue);
            } else {
     ...
     * @param {number} organizationIdValue 삭제 대상 organization_id (정수)
     ...
     *
     * @param {Array}  items        GET /relevance/mine 응답의 items 배열 (조직 판정만)
    ...
    function renderMyJudgmentsList(items, canonicalId) {
        ...
        items.forEach(function (item) {
            ...
            subjectSpan.textContent = item.organization_name || '(조직명 없음)';
     ...
     *
     * @param {string} canonicalId    canonical_project PK
     * @param {number} organizationId 삭제 대상 organization_id
     */
        ...
        .then(function (resp) {
            ...
            loadMyJudgmentsList(canonicalId, function (items) {
                if (currentInvoker && currentInvoker.classList && currentInvoker.classList.contains('rj-wrap')) {
                    if (items.length === 0) {
                        updateBadgeWrap(currentInvoker, '', '', null);
                    } else {
                        var first = items[0];
                        updateBadgeWrap(currentInvoker, first.verdict, first.reason || '', first.organization_id || null);
                    }
                }
            });
        })
     ...
     * - verdict='' 는 삭제 — 미검토로 되돌린다.
     * - verdict 가 있는 경우 항상 조직 판정(origin='organization') 으로 기록한다.
     *
     * @param {HTMLElement}  wrapEl           .rj-wrap 컨테이너
     * @param {string}       verdict          '관련' | '무관' | '' (삭제)
     * @param {string}       reason           사유 텍스트
     * @param {number|null}  organizationIdValue  조직 PK (삭제 시 null)
     */
    function updateBadgeWrap(wrapEl, verdict, reason, organizationIdValue) {
        wrapEl.dataset.myVerdict = verdict;
        wrapEl.dataset.myReason = reason || '';
        if (verdict) {
            wrapEl.dataset.myOrigin = 'organization';

...
> 현재는 **접수중** 상태의 공고만 수집한다.

## 구성 개요

- **Scraper**: Playwright headless Chromium 으로 IRIS 목록·상세 및 첨부파일(pdf/hwp/hwpx/zip) 수집
- **DB**: SQLAlchemy 2.0 + SQLite (기본). 공고와 첨부파일 메타데이터를 적재
- **Web**: FastAPI + Jinja2 로 로컬 전용 열람 UI 및 첨부 다운로드 API 제공
- **Config**: pydantic-settings 기반 `.env` 로더
...
│   ├── scraper/                # list/detail scraper + 첨부 downloader
│   ├── services/               # (예약)
...
| 변수 | 기본값 | 설명 |
| --- | --- | --- |
| `BASE_URL` | IRIS 목록 URL | 스크래핑 대상 URL |
| `REQUEST_DELAY_SEC` | `1.5` | 요청 간 지연(초). 차단 방지용 |
| `NAVIGATION_TIMEOUT_MS` | `30000` | Playwright 네비게이션 타임아웃 |
| `HEADLESS` | `true` | 브라우저 headless 여부. 디버깅 시 false |
| `USER_AGENT` | (빈값) | 커스텀 User-Agent. 비우면 Playwright 기본값 |
...
기본 명령은 `python -m app.cli run --status 접수중` 이며, `접수중` 상태의
공고 목록 → 상세 → 첨부파일 다운로드 순으로 처리해 SQLite DB 에 적재한다.
인자를 바꾸고 싶다면 뒤에 덧붙일 수 있다. 예:

```bash
# 1 페이지만 스캔하되 실제 DB/파일 쓰기 없이 드라이런
docker compose --profile scrape run --rm scraper \
  python -m app.cli run --status 접수중 --max-pages 1 --dry-run
```

수집된 데이터는 호스트의 `./data/db/app.sqlite3` 와 `./data/downloads/<announcement>/…`
...
- **첨부파일 저장소**: `./data/downloads/<announcement_slug>/...`

## 주의사항

- **로컬 전용.** FastAPI 백엔드는 인증·권한 제어가 없다. 외부에 노출하지 말 것.
- **차단 방지.** IRIS 가 동일 IP 의 과도한 요청을 차단할 수 있다. `REQUEST_DELAY_SEC`
  (기본 1.5초) 를 너무 짧게 설정하지 말고, `--max-pages` 로 범위를 제한해서 사용할 것.
...
- **개인정보/민감정보.** 본 프로젝트는 IRIS 공개 공고만 수집하지만, 저장된 첨부파일에
...
1. **환경 구성**
   - [ ] `.env` 가 생성되어 있고 `BASE_URL`, `DB_URL`, `DOWNLOAD_DIR` 이 의도한 값이다.
...
3. **스크래퍼 드라이런**
   - [ ] `docker compose --profile scrape run --rm scraper python -m app.cli run --status 접수중 --max-pages 1 --dry-run`
         실행 시 브라우저 부팅 → 목록 파싱 → 상세 파싱 로그가 정상 출력되고
         DB/파일 쓰기 없이 exit code 0 으로 종료된다.
4. **스크래퍼 실제 실행**
   - [ ] `docker compose --profile scrape run --rm scraper` (또는 `--max-pages 1`) 실행 후
         `./data/db/app.sqlite3` 가 생성되고, `./data/downloads/…` 밑에 첨부파일이 저장된다.
   - [ ] 같은 명령을 재실행해도 중복 공고가 생기지 않는다(UPSERT 동작 확인).
5. **웹 UI 기능 확인**

...
├── tests/
├── .env.example
├── sources.yaml.template       # 수집 설정 템플릿 — 초기 설정 시 sources.yaml 로 복사해 사용
│                               # (sources.yaml 은 .gitignore 대상: 브랜치 전환 시 로컬 수정 보존 목적)
├── docker-compose.yml
├── pyproject.toml
└── README.md
```
...
`.env.example` §1 의 **필수 호스트 통합 변수** (`HOST_UID`/`HOST_GID`,
`HOST_DOCKER_GID`, `HOST_PROJECT_DIR`) 는 호스트마다 다르며 자동 fallback 이 없다.
채우는 절차는 [README.USER.md → 시작하기](README.USER.md#시작하기) 참조.
...
`sources.yaml` 은 `.gitignore` 대상이므로 저장소에는 포함되지 않는다
(브랜치 전환 시 로컬 수정이 롤백되지 않도록 git 추적에서 제외한 것이다).

...
- **도메인 모델**: `Announcement` 1:N `Attachment`. 공고 UPSERT 키는 `(source_type, source_announcement_id)` 복합 UNIQUE. `is_current=True`인 row가 현재 버전. 상태는 `AnnouncementStatus` Enum(`접수중/접수예정/마감`, 한글 원문 값).
...
- **상태 전이 TODO**: `docs/status_transition_todo.md` — IRIS 접수예정·마감 수집 시작 시 및 NTIS 등 신규 크롤러 구현 시 참고할 미구현 동작 정리.

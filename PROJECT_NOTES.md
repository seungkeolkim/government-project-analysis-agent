...
- **레이어 구조** (`app/` 패키지):
  ...
  - `app/scraper/list_scraper.py` — **httpx 기반** IRIS AJAX API(`retrieveBsnsAncmBtinSituList.do`) POST 직접 호출. Playwright 없음.
  - `app/scraper/detail_scraper.py` — **httpx + BeautifulSoup 기반** 상세 페이지 수집. IRIS 상세는 SSR(서버사이드 렌더링)이라 Playwright 불필요. `div.tstyle_view` 파싱 후 HTML·텍스트·수집 시각 반환.
  - `app/web/` — FastAPI + Jinja2 템플릿(`templates/`) + 로컬 정적 CSS(`static/`). 목록(`list.html`)과 **상세 페이지(`detail.html`)** 라우트 모두 활성화.
...
- **데이터 흐름** (현재 활성):
  ...
  3. `scrape_details` 커맨드로 `detail_scraper`(httpx+BS4) 실행 → `detail_html`, `detail_text`, `detail_fetched_at`, `detail_fetch_status` DB 갱신
  4. 웹 UI는 목록 조회 + 공고 클릭 → 상세 페이지(`/detail/{id}`) 표시.
...
- **외부 의존성**: httpx(목록·상세 수집), BeautifulSoup4(상세 HTML 파싱), SQLAlchemy 2.0, FastAPI, uvicorn[standard], Jinja2, pydantic 2 + pydantic-settings, python-dotenv, loguru. Playwright는 첨부 수집 단계(task 00004+) 활성화 시 재도입 예정.
- **첨부 다운로드 설계**: `docs/attachment_download_plan.md` 에 상세 설계 기록. 첨부 링크는 `javascript:f_bsnsAncm_downloadAtchFile(...)` 형태 — AJAX POST(`/comm/file/retrieveCheckFileDownload.do`) 인증 후 실제 다운로드. Playwright headless 방식으로 구현 예정(task 00004).

...
- **레이어 구조** (`app/` 패키지):
  ...
  - `app/auth/` — Phase 1b 에서 추가된 인증 패키지. `constants.py`(쿠키/세션/비밀번호 정책 상수), `service.py`(passlib bcrypt 해시 + `create_user`/`authenticate`/`create_session`/`get_active_session`/`delete_session` + policy validators), `dependencies.py`(`current_user_optional`/`current_user_required` 두 버전 + `ensure_same_origin` CSRF-lite 체크), `routes.py`(`/register`, `/login`, `/auth/register|login|logout|me` 라우터 — `app/web/main.py` 가 `include_router` 로 마운트). 비로그인 열람을 기본 전제로 유지한다.
...
- **실행 환경**: Docker Compose 전용 (`docker compose --profile scrape run --rm scraper`). 호스트 Python 직접 실행 완전 제거. `app`(FastAPI UI)·`scraper`(1회성 배치, `profiles: [scrape]`)가 같은 이미지를 공유. 바인드 마운트: `./data`, `./app`, `./sources.yaml:/run/config/sources.yaml:ro`. `docker/entrypoint.sh`가 마운트된 `sources.yaml`을 per-run 임시 디렉터리(`mktemp`)로 복사하고 `SOURCES_CONFIG_PATH` 환경변수를 주입한 뒤 `python -m app.cli`를 exec — 동시 다중 실행 시 설정 경합 방지. **sources.yaml 우선순위**: (1) compose 바인드 마운트 → (2) 이미지 내 `sources.yaml.template` 폴백(최초 기동 시 경고 출력). 실행 파라미터는 CLI 인자 없이 `sources.yaml`의 `scrape:` 섹션으로만 제어.
...
- **외부 의존성**: httpx(목록·상세 수집), BeautifulSoup4(상세 HTML 파싱), pyyaml(sources.yaml 로드), SQLAlchemy 2.0, **Alembic 1.x**(스키마 마이그레이션), FastAPI, uvicorn[standard], Jinja2, pydantic 2 + pydantic-settings, python-dotenv, loguru, **playwright**(첨부 다운로드 — headless Chromium), **passlib[bcrypt] + bcrypt**(Phase 1b 비밀번호 해시), **python-multipart**(FastAPI Form/File 파싱 — 로그인·회원가입 multipart/form-data). Dockerfile에 `playwright install chromium --with-deps` 포함.

...
- **Backfill/검증 스크립트**: `scripts/backfill_canonical.py`(기존 IRIS 데이터 일회성 canonical 재계산), `scripts/verify_canonical_iris.py`(IRIS 단독 중복 케이스 검증).
...
- **`compute_canonical_key` 공식 키 우선 + fuzzy fallback**: official 키(ancmNo 정규화)를 우선 사용하고, 없으면 (제목 불용어 제거 + 주관기관 정규화 + 마감연도) 조합. 향후 NTIS cross-source 실매칭에서도 동일 로직 재사용 예정.
...
- [00014] NTIS 스크래퍼 실제 구현: httpx SSR 목록·상세 수집, NTIS 상태 P/B/Y 정규화, 첨부 POST 다운로드, `NtisSourceAdapter` 완전 구현, cross-source canonical 매칭 검증(NTIS fuzzy→official 승급), attachment downloader 소스 분기 adapter 계층 처리, NTIS viewer 템플릿 활성화, `sources.yaml` NTIS `enabled: true` 설정. — 2026-04-21
- [00013] canonical_key 레이어 도입: `CanonicalProject` 테이블·`canonical_group_id`·`canonical_key`·`canonical_key_scheme` 컬럼, `compute_canonical_key()`(official/fuzzy), UPSERT 시 자동 canonical 생성/매칭, backfill 스크립트, 검증 스크립트(`verify_canonical_iris.py`), `canonical_identity_design.md` 설계 문서. — 2026-04-21

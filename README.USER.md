...
즐겨찾기는 **canonical 과제 단위**로 저장된다 — IRIS·NTIS 어느 쪽에서 저장해도 같은 과제면 한 번만 기록된다.

### 별 아이콘 (☆ / ★)

목록 페이지와 상세 페이지의 제목 왼쪽에 별 아이콘이 표시된다 (로그인 + canonical 그룹이 있는 공고에만).

| 아이콘 | 의미 |
|--------|------|
| ☆ (빈 별) | 아직 즐겨찾기하지 않음 |
| ★ (채워진 별) | 즐겨찾기됨 (어느 폴더에든 저장된 상태) |
...
4. **추가** 버튼을 누르면 즐겨찾기가 저장되고 별이 ★ 으로 바뀐다.
5. 같은 canonical 그룹의 다른 소스 공고(예: NTIS)에서도 별이 ★ 로 동기화된다.

**즐겨찾기 제거 방법**:

- 목록 또는 상세 페이지: 채워진 별(★)을 클릭하면 즉시 제거된다.
...
| 폴더 이름 변경 | 폴더 행에 마우스 올리면 ✎ 아이콘 노출 → 클릭 → 다이얼로그에서 변경 |
| 폴더 삭제 | 폴더 행에 마우스 올리면 ✕ 아이콘 노출 → 클릭 → 확인 다이얼로그 후 삭제 |

> **폴더 깊이 제한**: 그룹 > 서브그룹 2단계까지만 허용된다. 서브그룹을 선택한 상태에서는 **서브그룹 추가** 버튼이 비활성화된다.

> **폴더 삭제 시 cascade**: 폴더를 삭제하면 그 안의 모든 즐겨찾기 항목도 함께 삭제된다. 확인 다이얼로그를 통해 이중 확인한다.

**우 패널 — 공고 목록**:

좌 패널에서 폴더를 선택하면 해당 폴더에 저장된 즐겨찾기 항목이 공고 테이블로 표시된다.
관련성 배지(미검토 / 관련 / 무관), 소스, 상태, 마감일 컬럼이 포함되며, **제거** 버튼으로 개별 항목을 삭제할 수 있다.
